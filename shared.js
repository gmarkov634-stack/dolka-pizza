...
export async function api(p,o={}){const r=await fetch(url(p),{...o,headers:{'Content-Type':'application/json',...(o.headers||{})}});const t=r.headers.get('content-type')||'';const b=t.includes('application/json')?await r.json():await r.text();if(!r.ok){const e=new Error(b&&typeof b==='object'&&b.error?b.error:`Ошибка HTTP ${r.status}`);e.status=r.status;e.body=b;throw e}return b}
...
export function saveAccess(id,token,phone=''){if(!id)return;let a=[];try{a=JSON.parse(localStorage.getItem(ORDER_KEY)||'[]');if(!Array.isArray(a))a=[]}catch{}const old=a.find(x=>String(x.orderId)===String(id))||{};const trackingToken=String(token||old.trackingToken||'').trim();const savedPhone=phoneDigits(phone)||phoneDigits(old.phone||'');if(!trackingToken&&!savedPhone)return;a=a.filter(x=>String(x.orderId)!==String(id));a.unshift({orderId:id,trackingToken,phone:savedPhone,savedAt:Date.now()});localStorage.setItem(ORDER_KEY,JSON.stringify(a.slice(0,20)))}
